<template>
  <div class="course-classes-page page-container">
    <h1 class="page-title">教学班级管理</h1>
    
    <!-- 搜索栏 -->
    <div class="search-form">
      <el-input
        v-model="searchParams.keyword"
        placeholder="搜索课程名称/编码/教师姓名"
        clearable
        style="width: 250px"
      />
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑教学班' : '新增教学班'" width="500px">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="课程编码" prop="code">
          <el-input v-model="form.code" :disabled="isEdit" placeholder="请输入课程编码" />
        </el-form-item>
        <el-form-item label="授课教师" prop="teacherId">
          <el-select v-model="form.teacherId" placeholder="请选择授课教师" clearable filterable>
            <el-option
    ...
    <!-- 学生管理弹窗 -->
    <el-dialog v-model="studentsDialogVisible" title="教学班学生管理" width="900px">
      <div class="students-toolbar">
        ...
        />
        <el-select v-model="studentStatus" placeholder="选择状态" clearable style="width: 120px">
        ...
        <el-table-column label="操作" width="150">
          <template #default="{ row }">
            <template v-if="row.status === 'pending'">
              <el-button type="success" link @click="approveStudent(row.id)">通过</el-button>
              <el-button type="warning" link @click="rejectStudent(row.id)">拒绝</el-button>
            </template>
            <el-popconfirm
              title="确定要移除该学生吗？"
...
const handleEdit = (item) => {
  ...
  form.maxStudents = item.maxStudents || 50
...
const handleSave = async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      await updateCourseClass(currentClassId.value, form)
    } else {
      await createCourseClass(form)
    }
...
const handleBatchDelete = async () => {
  try {
    await ElMessageBox.confirm(
      `确定要删除选中的 ${selectedIds.value.length} 个教学班吗？`,
      '批量删除',
      { type: 'warning' }
    )
    
    const result = await batchDeleteCourseClasses(selectedIds.value)
    
    if (result.failed > 0) {
      ElMessage.warning(`删除完成：成功 ${result.success} 个，失败 ${result.failed} 个`)
    } else {
      ElMessage.success(`成功删除 ${result.success} 个教学班`)
    }
...
const loadClassStudents = async () => {
  ...
  try {
    const res = await getCourseClassStudents(currentClassId.value, {
...
const approveStudent = async (studentId) => {
  try {
    await approveStudentInCourseClass(currentClassId.value, studentId)
...
const rejectStudent = async (studentId) => {
  try {
    await rejectStudentInCourseClass(currentClassId.value, studentId)
...
const removeStudent = async (studentId) => {
  try {
    await removeStudentFromCourseClass(currentClassId.value, studentId)
