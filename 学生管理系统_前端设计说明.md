...
<template>
  <div class="user-management">
    ...
    <div class="toolbar">
      ...
      >
        批量删除 {{ selectionCount > 0 ? `(${selectionCount})` : '' }}
      </el-button>
    </div>
...
async function handleBatchDelete() {
  const result = await executeBatch(batchDeleteUsers, '删除成功')
  if (result) {
    await loadUserList()
  }
}
...
本前端设计说明文档定义了学生管理系统前端的技术架构、目录结构、路由权限、API 封装、批量操作等核心设计方案。

**本次更新要点：**
1. 完善目录结构，增加 composables、constants 等目录
2. 新增批量操作设计方案，包含组合式函数和页面示例
3. 增加错误码处理机制，与后端错误码对应
4. 补充性能优化和错误处理规范
5. 提出架构和功能层面的改进建议
